...
class BaseCAM:
    def __init__(self,
                 ...
                 uses_gradients=True):
        self.model = model.eval()                                                         # ΔΕΝ αλλαξα εδω προσοχή
    ...
    def forward(self, input_tensor, target_category=None, eigen_smooth=False):
        ...
        output = output # προσθήκη
        if isinstance(target_category, int):
            target_category = [target_category] * input_tensor.size(0)

        if target_category is None:
            target_category = np.argmax(output.cpu().data.numpy(), axis=-1)    #βαζω εξτρα [0]
        else:
            assert(len(target_category) == input_tensor.size(0))

        if self.uses_gradients:
            self.model.zero_grad()
            loss = self.get_loss(output, target_category)   # βάζω εξτρα [0]
    ...
    def compute_cam_per_layer(
        ...
        for target_layer, layer_activations, layer_grads in \
            ...
            cam = self.get_cam_image(input_tensor,
                                     ...
                                     layer_grads,
                                     eigen_smooth)
            cam[cam<0]=0 # works like mute the min-max scale in the function of scale_cam_image WE SIMPLY UNDO THIS LINE FOR NOW
